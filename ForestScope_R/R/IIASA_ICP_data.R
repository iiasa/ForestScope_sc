#' Process the ICP climate and soil database
#' This function process the loaded ICP,ISRIC and CHELSA data for model evalution/calibration or other purposes
#' @param climate: Climate data output
#' @param soil: Site soil data output
#' @param species: Site species data output
#' @param thinning: Site thinning data output
#' @return A list containing processed data frames for soil and climate data from all the loaded database
#' @import soiltexture
#' @examples
#'  IIASA_ICP_process(isric_soil_d, icp_data, site_number)
#' @export
#'

############################ Begin data preparation for site level ############################
                          ############# select site #############
                          ############# Request input #############


# Define IIASA_ICP_process function
IIASA_ICP_process <- function(isric_soil_database = NULL, icp_database = NULL, country= NULL, site_number= NULL) {

   library(soiltexture)

  # Check if isric_soil_d is NULL or not defined
  if (is.null(isric_soil_database) && !exists("isric_soil_d")) {
    source(file = "R/soil_data_prepration.R")
    # Call isric_soil_data function
    isric_soil_d <- isric_soil_data()
  } else {
    isric_soil_d <-  isric_soil_database
  }

  # Check if icp_data is NULL or not defined
  if (is.null(icp_database) && !exists("icp_data")) {
    source(file = "R/ICP_data_prepration.R")
    # Call ICP_climate_soil function
    icp_data <- ICP_climate_soil()
  } else {
    icp_data <- icp_database
  }

    if(is.null(country)){
    print(icp_data$country_names)
    country <- readline(prompt = "Select country:")
    # Select country
    if (country %in% icp_data$country_names) {
      select_country <- country
    } else {
      cat("Error: Country not found in ICP dataset\n")
      select_country <- readline(prompt = "Select country:")
    }
    }else{
      select_country <- country
    }

  # Check if selected country exists in icp_data list
  if (select_country %in% icp_data$country_names) {
    for (i in 1:38) {
      if (icp_data$lat_lon[[i]]$country_name== select_country) {
        selected_list <- icp_data$lat_lon[[i]]
        break
      }
    }
    # Pull data for the selected country
    lat_lon_names <- names(selected_list)
    lat_lon_names <- sub("^country_name$", "", lat_lon_names)
    lat_lon_names <- gsub("_", "", lat_lon_names)
    lat_lon_names <- sub("sitenumNA", "", lat_lon_names)

    lat_lon_nums <- as.numeric((gsub(".*sitenum[[:punct:]]*(\\d+).*", "\\1", lat_lon_names, ignore.case = TRUE)))

    # Remove any NA values from the resulting numeric vector
    lat_lon_nums <- lat_lon_nums[!is.na(lat_lon_nums)]
    available_sites<-as.numeric(na.omit(lat_lon_nums))
    cat("Available sites in the icp_data list:\n")
    print(available_sites)
  } else {
    # If selected country doesn't exist in icp_data list, show error message
    cat("Selected country is not available in the icp_data list.\n")
  }

  if(is.null(site_number)){
  select_site <- readline(prompt = "select site:")
  ss_select<-paste0("site_num_",select_site)}else{
  ...
  for (i in 1:length(x)){
    if(names(x[i]) == ss_select){
      x_s<- x[i]
      site_date_tmp <- x_s[[ss_select]]$date_tmp
      site_tmin <- x_s[[ss_select]]$tmin
      site_tmax <- x_s[[ss_select]]$tmax
      site_tavg <- x_s[[ss_select]]$tavg
    }
  }
  ...
  for (i in 1:length(x)){
    if(names(x[i]) == ss_select){
      x_s<- x[i]
      site_date_pr <- x_s[[ss_select]]$date_pr
      site_pr <- x_s[[ss_select]]$pr
    }
  }
  ...
  for (i in 1:length(x)){
    if(names(x[i]) == ss_select){
      x_s<- x[i]
      site_date_sr <- x_s[[ss_select]]$date_sr
      site_sr <- x_s[[ss_select]]$sr
    }
  }
  ...
  for (i in 1:length(x)){
    if(names(x[i]) == ss_select){
      x_s<- x[i]
      site_lat <- x_s[[ss_select]]$lat
      site_lon <- x_s[[ss_select]]$lon
    }
  }
  ...
  if(length(x_pr$date) >0 & (length(site_pr) == length(x_pr$date))){
    ...
    lat_min <- (abs(icp_data$chelsa_clim$pr$LON- as.numeric(unique(site_lon)))==min(abs(icp_data$chelsa_clim$pr$LON- as.numeric(unique(site_lon)))))
    lon_min <- (abs(icp_data$chelsa_clim$pr$LAT- as.numeric(unique(site_lat)))==min(abs(icp_data$chelsa_clim$pr$LAT- as.numeric(unique(site_lat)))))
    ch_pr_in <- icp_data$chelsa_clim$pr %>% filter( lat_min & lon_min)
  ...
  if(length(x_sr$date) >0 & (length(site_date_sr) == length(x_sr$date))){
    ...
    sradi<- aggregate((site_sr/10)~x_sr$month + x_sr$year, datess_sr,mean)}else{
      ...
      lat_min <- (abs(icp_data$chelsa_clim$sr$LON- as.numeric(unique(site_lon)))==min(abs(icp_data$chelsa_clim$sr$LON- as.numeric(unique(site_lon)))))
      lon_min <- (abs(icp_data$chelsa_clim$sr$LAT- as.numeric(unique(site_lat)))==min(abs(icp_data$chelsa_clim$sr$LAT- as.numeric(unique(site_lat)))))
      ch_rad_in <- icp_data$chelsa_clim$sr %>% filter( lat_min & lon_min)
  ...
  ss_select <-as.numeric(select_site)
  ...
  if(length(tt$survey_year) > 0 && any(complete.cases(tt$diameter)) && any(complete.cases(tt$height))){
    ...
    for(i in 1:length(sp)){
      bio_s[i] <-exp(beta0_s+beta1_s*log(mean(diam))+0.087*log(mean(height)))}
    root_s<- array(NA,length(sp))
    for(i in 1:length(sp)){
      root_s[i] <-exp(beta0_r+beta1_r*log(mean(diam))+0.087*log(mean(height)))}
    foliage_s<- array(NA,length(sp))
    for(i in 1:length(sp)){
      foliage_s[i] <-exp(beta0_f+beta1_f*log(mean(diam))+0.087*log(mean(height)))}
    if(length(exp(beta0_d+beta1_d*log(mean(diam))))>0){
      for(i in 1:length(sp)){
        deadwood_c[i] <-exp(beta0_d+beta1_d*log(mean(diam))+0.087*log(mean(height)))}}
    ...
    bio_s<- array(NA,length(sp))
    for(i in 1:length(sp)){
      bio_s[i] <-exp(beta0_s+beta1_s*log(mean(diam)))}
    root_s<- array(NA,length(sp))
    for(i in 1:length(sp)){
      root_s[i] <-exp(beta0_r+beta1_r*log(mean(diam)))}
    foliage_s<- array(NA,length(sp))
    for(i in 1:length(sp)){
      foliage_s[i] <-exp(beta0_f+beta1_f*log(mean(diam)))}
    if(length(exp(beta0_d+beta1_d*log(mean(diam))))>0){
      for(i in 1:length(sp)){
        deadwood_c[i] <-exp(beta0_d+beta1_d*log(mean(diam)))}}
  ...
  data <- data.frame( species=sp,	planted=paste0(pl,"-01"),	fertility=fer,	stems_n=st_n,	biom_stem=bio_s,	biom_root=root_s,	biom_foliage=foliage_s, deadwood_carbon=deadwood_c,	deadwood_nitrogen=deadwood_n,	litter_carbon=litter_c,	litter_nitrogen=litter_n,	n_sapling=n_sap,	sapling_wf=sap_wf,	sapling_wr=sap_wr,	sapling_ws=sap_ws)
  ...
  if(length(years_s)>0){
    for(i in 1:length(sp)){
      if ("tree_number" %in% names(tt)) {
        t_int_s<-mean(filter(tt,tt$survey_year==years_s[1])$trees_number,na.rm=T)}
    ...
    if(length(years_s >3)){
      for(i in 2:length(years_s)){
        if ("tree_number" %in% names(tt)) {
          t_d<-mean(filter(tt,tt$survey_year==years_s[i])$trees_number)}
        if ("trees_number" %in% names(tt)) {
          t_d<-mean(filter(tt,tt$survey_year==years_s[i])$trees_number)}
        if(!is.na(t_d)){
          if(t_d < t_int_s){age_init <- age_init + years_s[1] - pl[1:length(sp)]}}}}else{
            if(length(years_s==2)){
              if ("tree_number" %in% names(tt)) {
                t_d<-mean(filter(tt,tt$survey_year==years_s[2])$trees_number)}
              if ("trees_number" %in% names(tt)) {
                t_d<-mean(filter(tt,tt$survey_year==years_s[2])$trees_number)}
              if(!is.na(t_d)){
  ...
  data <- data.frame(species=sp,age=age_i,stems_n=ss_n,stem=s,root=r,foliage=f,BA=dd,Vol=V)
  d_thinning <- as_tibble(data)

  ######## End of thinning data preparation #####

   return(list(climate = d_climate, soil = d_site, species = d_species, thinning = d_thinning))
